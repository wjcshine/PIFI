...
data$C.G180=data$CP_E/data$GLU_E

data$top_CP=NA
data$top.time_CP=NA
for (i in c(1:nrow(data))){
  data$top_CP[i]=max(data$CP_A[i],data$CP_B[i],data$CP_C[i],data$CP_D[i],data$CP_E[i])
  data$top.time_CP[i]=which.max(c(data$CP_A[i],data$CP_B[i],data$CP_C[i],data$CP_D[i],data$CP_E[i]))
}
...
for (i in 1:nrow(data)){
  ...
  if(data$top.time_CP[i]==180){
    data$top_CP_G[i]=data$top_CP[i]/data$GLU_E[i]}
}
...
data$C_Ppeak_G=1966.9757-9.5129715*data$Height+0.054585423*pmax(data$Height-151.85,0)^3-0.14208178*pmax(data$Height-158.5,0)^3+0.09986513*pmax(data$Height-164.55,0)^3-0.012368768*pmax(data$Height-178,0)^3+30.283836*data$Sex-3.1911403*data$Age

data$`PIF-Sc`=data$C.G0/data$C_P0_G0*100
data$`PIF-Lc`=data$top_CP_G/data$C_Ppeak_G*100
data$`PIF-Ac`=data$slope_CP/0.07563291*100

data=select(data,-C.G0,-C.G30,-C.G60,-C.G120,-C.G180,
